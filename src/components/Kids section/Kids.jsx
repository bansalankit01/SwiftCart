        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-5 place-items-center">
          {ProductsData.map((product) => (
            <div
              key={product.id}
              data-aos="zoom-in"
              className="rounded-2xl text-center bg-white dark:bg-gray-800 hover:bg-black/80 dark:hover:bg-primary hover:text-white shadow-xl duration-300 group w-full sm:w-[200px] md:w-[250px] h-[350px] flex flex-col justify-between"
            >
              {/* Image Section */}
              <div className="h-[150px] flex items-center justify-center">
                <img
                  src={product.img}
                  alt={product.img}
                  className="max-w-[120px] sm:max-w-[140px] md:max-w-[150px] md:mt-16 mt-10 object-contain block mx-auto transform group-hover:scale-105 duration-300 drop-shadow-md"
                />
              </div>

              {/* Details Section */}
              <div className="p-4 flex flex-col justify-between flex-grow md:mt-16 mt-10">
                <h1 className="text-sm font-bold line-clamp-2">{product.title}</h1>

                <p className="dark:text-red-500 text-red-500 font-bold md:mt-2 mt-5 group-hover:text-red-500 duration-300 text-sm">
                  {product.price}
                </p>
