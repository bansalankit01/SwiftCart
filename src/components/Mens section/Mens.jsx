          ...
          {ProductsData.map((product) => (
            <div key={product.id} className="rounded-2xl text-center bg-white dark:bg-gray-800 hover:bg-black/80 dark:hover:bg-primary hover:text-white shadow-xl duration-300 group w-full sm:w-[200px] md:w-[250px] h-[350px] flex flex-col justify-between">
              <div className="h-[150px] flex items-center justify-center">
                <img src={product.img} alt={product.title} className="max-w-[140px] mt-16 md:max-w-[150px] object-contain block mx-auto transform group-hover:scale-105 duration-300 drop-shadow-md" />
              </div>
              <div className="p-4 flex flex-col justify-between flex-grow ">
                <h1 className="text-sm font-bold line-clamp-2 mt-11 ">{product.title}</h1>
                <p className="dark:text-red-500 text-red-500 font-bold group-hover:text-red-500 duration-300 text-sm mt-4">{product.price}</p>
                <button className="bg-primary hover:scale-105 duration-300 text-white py-1 rounded-md mt-auto group-hover:bg-white group-hover:text-primary"  onClick={() => addToCart(product)}>
                  Add to cart
                </button>
              </div>
            </div>
